...
def calculate_iou_partly(gt_annos, dt_annos, metric, num_parts=50):
    ...
    Args:
        gt_annos: dict, must from get_label_annos() in saite_common.py
        dt_annos: dict, must from get_label_annos() in saite_common.py
    ...
    for num_part in split_parts:
        ...
        if metric == 0:
            ...
            # translate camera to lidar
            trans = np.array([[0, -1, 0], [0, 0, -1], [1, 0, 0]])
            trans_inv = np.linalg.inv(trans)
            gt_loc_ = []
            for a in gt_annos_part:
                loc_part = np.matmul(a['location'], trans_inv)
                gt_loc_.append(loc_part)
            loc = np.concatenate(gt_loc_, 0)
            dims = np.concatenate([a["dimensions"] for a in gt_annos_part], 0)
            rots = np.concatenate([a["rotation_y"] for a in gt_annos_part], 0)
            ...
            for a in dt_annos_part:
                loc_part = np.matmul(a['location'], trans_inv)
                dt_loc_.append(loc_part)
            loc = np.concatenate(dt_loc_, 0)
            dims = np.concatenate([a["dimensions"] for a in dt_annos_part], 0)
            rots = np.concatenate([a["rotation_y"] for a in dt_annos_part], 0)
...
def get_official_eval_result(gt_annos, dt_annos, current_classes, PR_detail_dict=None):
    ...
    overlap_0_5 = np.array([[0.5, 0.5, 0.5, 0.5, 0.5, 0.5],
                            ...
                            [0.5, 0.5, 0.5, 0.5, 0.5, 0.5]])
    overlap_0_7 = np.array([[0.7, 0.7, 0.7, 0.7, 0.7, 0.7],
                            [0.7, 0.7, 0.7, 0.7, 0.7, 0.7],
                            [0.7, 0.7, 0.7, 0.7, 0.7, 0.7]])

    min_overlaps = np.stack([overlap_0_3, overlap_0_5, overlap_0_7], axis=0)  # [2, 3, 5]
    class_to_name = {
        0: 'Car',
        1: 'Pedestrian',
        2: 'Cyclist',
    ...
    mAP3d, mAP3d_R40 = do_eval(gt_annos, dt_annos, current_classes, min_overlaps, compute_aos,
    ...
    for j, curcls in enumerate(current_classes):
        ...
        for i in range(min_overlaps.shape[0]):
            result += print_str(
                f"{class_to_name[curcls]} ", "AP@{:.2f}, {:.2f}, {:.2f}:".format(*min_overlaps[i, :, j]))
